...
local function write_settings_file(filepath, settings)
...
-- Save current settings as new preset / copy
function Presets:save(settings)
    ...
    if write_settings_file(filepath, settings) then
...
-- Overwrite currently selected preset, or save as new if none is selected
function Presets:overwrite(settings)
    ...
    if write_settings_file(filepath, settings) then
        print("Overwrote preset: " .. filename)
        return true
    end
